...
When an order is created, it is added to ImmutableX's orderbook smart contract on StarkEx. This shared orderbook is what enables all orders created on ImmutableX to be available on every marketplace built on this protocol.
...
In order to use the Core SDK, you need to [initialize it](./how-to-install-initialize#core-sdk).

### 2. Generate signers
Creating an order for a user requires a user's signature, so your application will need to create signers. See the guide on [how to generate signers](./how-to-generate-signers).

### 3. Set the order params

:::note When setting [maker fees](./fees) in the order params
