---
title: "Asset transfers"
slug: "/guides-new/transfers"
keywords: [imx-wallets]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

A user can transfer the ownership of an asset that they own to another user.

### Guides:
* [Asset transfers using the Core SDK](#asset-transfers-using-the-core-sdk)
* [Asset transfers on StarkNet](#asset-transfers-on-starknet)

## Asset transfers using the Core SDK

### 1. Initialize the Core SDK
In order to use the Core SDK, you need to [initialize it](./get-started#getting-started-with-the-core-sdk).

### 2. Generate signers
Transferring an asset requires a user's signature, so your application will need to [generate signers](/docs/guides-new/generate-signers).

### 3. Set the transfer params
<Tabs>
  <TabItem value="typescript" label="Typescript Core SDK">
    Add description, if required.
    <br/><br/>

```ts
// Add code
```
  </TabItem>
  <TabItem value="kotlin" label="Kotlin (JVM) Core SDK">
    Add description, if required.
    <br/><br/>

```kotlin
// Add code
``` 
  </TabItem>
  <TabItem value="Swift" label="Swift Core SDK">
    Add description, if required.
    <br/><br/>

```swift
// Add code
```
  </TabItem>
  <TabItem value="go" label="Golang Core SDK">
    Add description, if required.
    <br/><br/>

```go
// Add code
```
  </TabItem>
</Tabs>

### 4. Create the transfer
<Tabs>
  <TabItem value="typescript" label="Typescript Core SDK">
    Add description, if required.
    <br/><br/>

```ts
// Add code
```
  </TabItem>
  <TabItem value="kotlin" label="Kotlin (JVM) Core SDK">
    Add description, if required.
    <br/><br/>

```kotlin
// Add code
``` 
  </TabItem>
  <TabItem value="Swift" label="Swift Core SDK">
    Add description, if required.
    <br/><br/>

```swift
// Add code
```
  </TabItem>
  <TabItem value="go" label="Golang Core SDK">
    Add description, if required.
    <br/><br/>

```go
// Add code
```
  </TabItem>
</Tabs>

## Asset transfers on StarkNet


