...
#### _Asset.sol_
[Asset.sol](https://github.com/immutable/imx-contracts/blob/main/contracts/Asset.sol) is an example of a non-fungible (ERC-721 standard) contract that contains the [functions required](./deep-dive-minting#l1-contract-requirements) so that tokens can be minted from it on ImmutableX.
...
Currently, L2 minting of fungible tokens are not supported by ImmutableX. To transact with these tokens on L2, they must be minted first on L1 then [deposited](./how-to-enable-deposits-withdrawals) to L2 to be used.
...
More information:
* For a conceptual overview, see [Deep dive into minting](./deep-dive-minting)
* [Guide on how to use our API/SDKs to mint](./how-to-mint-assets)
